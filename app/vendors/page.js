...
export default function VendorsPage() {
  ...
  const [invoiceForm, setInvoiceForm] = useState({ invoice_number: "", amount: "", date: "", notes: "" });
  ...
  const openEditForm = (vendor) => { setEditingVendor(vendor); setShowForm(true); };
  const closeForm = () => { setEditingVendor(null); setShowForm(false); };

  const openInvoiceModal = (vendor) => {
    setInvoiceVendor(vendor);
    setInvoiceForm({ invoice_number: "", amount: "", date: new Date().toISOString().slice(0, 10), notes: "" });
  ...
  const exportCSV = () => {
    ...
    if (exportDateFrom) exportVendors = exportVendors.filter((v) => new Date(v.created_at) >= new Date(exportDateFrom));
    if (exportDateTo) {
    ...
    const rows = exportVendors.map((v) => [v.name ?? "", v.contact ?? "", v.notes ?? "", v.created_at ? new Date(v.created_at).toLocaleDateString() : ""]);
  ...
  return (
    <div className="page">
      ...
      {loading ? (
        ...
        <div className="vendor-grid">
          {vendors.map((v) => {
            ...
            const totalInvoiced = vInvoices.reduce((s, i) => s + (i.amount || 0), 0);
            return (
              <div key={v.id} className="vendor-card">
                ...
                <div className="invoice-summary">
                  <div className="invoice-summary-row">
                    <span className="invoice-count">{vInvoices.length} invoice{vInvoices.length !== 1 ? "s" : ""}</span>
                    {totalInvoiced > 0 && <span className="invoice-total">${totalInvoiced.toFixed(2)} total</span>}
                    <button className="btn-action btn-invoice" onClick={() => openInvoiceModal(v)}>+ Invoice</button>
                    {vInvoices.length > 0 && (
                      <button className="btn-action btn-view" onClick={() => setExpandedVendor(expandedVendor === v.id ? null : v.id)}>
                      ...
                      {vInvoices.map((inv) => (
                        <div key={inv.id} className="invoice-row">
                          <div className="invoice-info">
                            ...
                            {inv.amount && <span className="invoice-amt">${Number(inv.amount).toFixed(2)}</span>}
                            {inv.notes && <span className="invoice-note">{inv.notes}</span>}
                          </div>
                          <button className="btn-action btn-delete" onClick={() => deleteInvoice(inv.id)}>✕</button>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      {showForm && (
        <div className="modal-overlay" onClick={closeForm}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <VendorForm vendor={editingVendor} onSuccess={() => { closeForm(); fetchAll(); }} onCancel={closeForm} />
          </div>
        </div>
      )}
          ...
          <div className="modal-box modal-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="inv-body">
              <label>Invoice Number *
                <input placeholder="e.g. INV-1042" value={invoiceForm.invoice_number} onChange={(e) => setInvoiceForm(f => ({ ...f, invoice_number: e.target.value }))} />
              </label>
              <label>Date *
                <input type="date" value={invoiceForm.date} onChange={(e) => setInvoiceForm(f => ({ ...f, date: e.target.value }))} />
              </label>
              <label>Amount ($)
                <input type="number" min="0" step="0.01" placeholder="e.g. 125.00" value={invoiceForm.amount} onChange={(e) => setInvoiceForm(f => ({ ...f, amount: e.target.value }))} />
              </label>
              <label>Notes (optional)
                <input placeholder="e.g. Canned goods delivery" value={invoiceForm.notes} onChange={(e) => setInvoiceForm(f => ({ ...f, notes: e.target.value }))} />
              </label>
              <div className="modal-actions">
                <button className="btn btn-secondary" onClick={() => setShowInvoiceModal(false)}>Cancel</button>
                <button className="btn btn-primary" onClick={handleAddInvoice} disabled={invoiceLoading}>
      ...
      {showExportModal && (
        <div className="modal-overlay" onClick={() => setShowExportModal(false)}>
          <div className="modal-box modal-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="inv-body">
              <p style={{ fontSize: "0.875rem", color: "#4b5563", margin: 0 }}>Filter by date added. Leave blank to export all vendors.</p>
              <label>From date<input type="date" value={exportDateFrom} onChange={(e) => setExportDateFrom(e.target.value)} /></label>
              <label>To date<input type="date" value={exportDateTo} onChange={(e) => setExportDateTo(e.target.value)} /></label>
              <div className="modal-actions">
      ...
      <style jsx>{`
        ...
        .vendor-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 16px; padding: 24px 36px; }
        ...
        .btn-view { background: #f5f3ff; color: #7c3aed; }
        .invoice-summary { margin-top: 12px; padding-top: 12px; border-top: 1px solid #f0f0f0; }
        .invoice-summary-row { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
        .invoice-count { font-size: 0.78rem; color: #6b7280; }
        .invoice-total { font-size: 0.78rem; font-weight: 700; color: #059669; background: #ecfdf5; padding: 2px 8px; border-radius: 999px; }
        .invoice-list { margin-top: 10px; display: flex; flex-direction: column; gap: 6px; }
        .invoice-row { display: flex; align-items: center; justify-content: space-between; background: #f9fafb; border-radius: 8px; padding: 8px 10px; }
        .invoice-info { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; flex: 1; }
        .invoice-num { font-weight: 700; font-size: 0.82rem; color: #1a1a2e; }
        .invoice-date { font-size: 0.78rem; color: #6b7280; }
        .invoice-amt { font-size: 0.82rem; font-weight: 600; color: #059669; }
        .invoice-note { font-size: 0.78rem; color: #9ca3af; font-style: italic; }
        ...
        .inv-body { padding: 20px 24px 24px; display: flex; flex-direction: column; gap: 14px; }
        .inv-body label { display: flex; flex-direction: column; gap: 5px; font-size: 0.8rem; font-weight: 600; color: #374151; text-transform: uppercase; letter-spacing: 0.03em; }
        .inv-body input { padding: 9px 12px; border: 1.5px solid #d1d5db; border-radius: 8px; font-size: 0.9rem; font-family: inherit; outline: none; }
        .inv-body input:focus { border-color: #861f41; }
